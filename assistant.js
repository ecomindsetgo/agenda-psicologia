...
(function () {
  ...
  function answer(intent, question) {
    ...
    if (intent === 'today' || intent === 'people' || intent === 'count') {
      ...
      const projection = list.filter(isActiveAppointment);
      return `<div class="assistant-title">📈 Proyección de ingresos ${title}</div><div class="assistant-total">${formatTotals(sumByCurrency(projection))}</div><div class="mt-1 text-slate-500">${projection.length} cita(s) consideradas: cobradas + pendientes.</div>`;
